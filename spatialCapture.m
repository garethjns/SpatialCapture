function [isRunning, stimLog, params, stimOrder] = spatialCapture(params)
% Main function
...
try
    for n = 1:params.n
        ...
        if breakTime(params, n)
            figs = exInBreak(params, stimOrder, stimLog, figs);
            % Includes save etc
        end
    ...
    % Save
    OK = writeMat(params, stimOrder, stimLog);
...
function figs = exInBreak(params, stimOrder, stimLog, figs)
...
% Save so far
writeMat(params, stimOrder, stimLog);
...
function OK = writeMat(params, stimOrder, stimLog) %#ok<INUSD> (are used)
% Save data collected so far to disk

OK = saveHandler(params.saveFile2, 1, 0, {'stimOrder', 'stimLog'});
